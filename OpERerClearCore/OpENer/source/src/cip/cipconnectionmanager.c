...
typedef struct {
  ...
  CipUdint buf_size_remaining;
} ConnectionManagerInstanceData;

static ConnectionManagerInstanceData g_connection_manager_instance_data;

static CipConnectionManagerConnectionEntryList g_connection_entry_list = {0, NULL};

static void EncodeConnectionEntryList(const void *const data, ENIPMessage *const outgoing_message) {
  (void)data;
  extern DoublyLinkedList connection_list;
  
  DoublyLinkedListNode *node = connection_list.first;
  CipUint num_conn_entries = 0;
  while(node != NULL) {
    CipConnectionObject *conn = node->data;
    if(conn != NULL && ConnectionObjectGetState(conn) == kConnectionObjectStateEstablished) {
      num_conn_entries++;
    }
    node = node->next;
  }
  
  g_connection_entry_list.num_conn_entries = num_conn_entries;
  AddIntToMessage(g_connection_entry_list.num_conn_entries, outgoing_message);
  
  CipUint conn_open_bits = 0;
  CipUint bit_index = 0;
  node = connection_list.first;
  while(node != NULL && bit_index < 32) {
    CipConnectionObject *conn = node->data;
    if(conn != NULL && ConnectionObjectGetState(conn) == kConnectionObjectStateEstablished) {
      conn_open_bits |= (1U << bit_index);
    }
    bit_index++;
    node = node->next;
  }
  AddDintToMessage(conn_open_bits, outgoing_message);
}
...
EipStatus ConnectionManagerInit(EipUint16 unique_connection_id) {
  ...
  g_connection_manager_instance_data.open_requests = 0;
  g_connection_manager_instance_data.open_format_rejects = 0;
  g_connection_manager_instance_data.open_resource_rejects = 0;
  g_connection_manager_instance_data.open_other_rejects = 0;
  g_connection_manager_instance_data.close_requests = 0;
  g_connection_manager_instance_data.close_format_requests = 0;
  g_connection_manager_instance_data.close_other_requests = 0;
  g_connection_manager_instance_data.connection_timeouts = 0;
  g_connection_manager_instance_data.cpu_utilization = 0;
  g_connection_manager_instance_data.max_buff_size = 4096U;
  g_connection_manager_instance_data.buf_size_remaining = 4096U;
  ...
  if(instance != NULL) {
    InsertAttribute(instance, 1, kCipUint, EncodeCipUint, NULL,
                    &g_connection_manager_instance_data.open_requests,
                    kGetableSingleAndAll);
    InsertAttribute(instance, 2, kCipUint, EncodeCipUint, NULL,
                    &g_connection_manager_instance_data.open_format_rejects,
                    kGetableSingleAndAll);
    InsertAttribute(instance, 3, kCipUint, EncodeCipUint, NULL,
                    &g_connection_manager_instance_data.open_resource_rejects,
                    kGetableSingleAndAll);
    InsertAttribute(instance, 4, kCipUint, EncodeCipUint, NULL,
                    &g_connection_manager_instance_data.open_other_rejects,
                    kGetableSingleAndAll);
    InsertAttribute(instance, 5, kCipUint, EncodeCipUint, NULL,
                    &g_connection_manager_instance_data.close_requests,
                    kGetableSingleAndAll);
    InsertAttribute(instance, 6, kCipUint, EncodeCipUint, NULL,
                    &g_connection_manager_instance_data.close_format_requests,
                    kGetableSingleAndAll);
    InsertAttribute(instance, 7, kCipUint, EncodeCipUint, NULL,
                    &g_connection_manager_instance_data.close_other_requests,
                    kGetableSingleAndAll);
    InsertAttribute(instance, 8, kCipUint, EncodeCipUint, NULL,
                    &g_connection_manager_instance_data.connection_timeouts,
                    kGetableSingleAndAll);
    InsertAttribute(instance, 9, kCipAny, EncodeConnectionEntryList, NULL,
                    &g_connection_entry_list, kGetableSingleAndAll);
    static const CipUint reserved_attr_10 = 0;
    InsertAttribute(instance, 10, kCipUint, EncodeCipUint, NULL,
                    (void *)&reserved_attr_10, kNotSetOrGetable);
    InsertAttribute(instance, 11, kCipUint, EncodeCipUint, NULL,
                    &g_connection_manager_instance_data.cpu_utilization,
                    kGetableSingleAndAll);
    InsertAttribute(instance, 12, kCipUdint, EncodeCipUdint, NULL,
                    &g_connection_manager_instance_data.max_buff_size,
                    kGetableSingleAndAll);
    InsertAttribute(instance, 13, kCipUdint, EncodeCipUdint, NULL,
                    &g_connection_manager_instance_data.buf_size_remaining,
                    kGetableSingleAndAll);
  }
...
EipStatus AssembleForwardOpenResponse(CipConnectionObject *connection_object,
                                      ...
                                      EipUint8 general_status,
                                      EipUint16 extended_status) {
  if(general_status == kConnectionManagerGeneralStatusSuccess) {
    g_connection_manager_instance_data.open_requests++;
  } else {
    if(general_status == kConnectionManagerGeneralStatusErrorInDataSegment ||
       general_status == kConnectionManagerGeneralStatusPathSegmentErrorInUnconnectedSend) {
      g_connection_manager_instance_data.open_format_rejects++;
    } else if(extended_status == kConnectionManagerExtendedStatusCodeErrorNoMoreConnectionsAvailable ||
              extended_status == kConnectionManagerExtendedStatusCodeNoBufferMemoryAvailable) {
      g_connection_manager_instance_data.open_resource_rejects++;
    } else {
      g_connection_manager_instance_data.open_other_rejects++;
    }
...
EipStatus AssembleForwardCloseResponse(EipUint16 connection_serial_number,
  ...
  if(extended_error_code == 0) {
    ...
    if(extended_error_code == kConnectionManagerExtendedStatusCodeErrorParameterErrorInUnconnectedSendService ||
       extended_error_code == kConnectionManagerExtendedStatusCodeErrorInvalidSegmentTypeInPath) {
      g_connection_manager_instance_data.close_format_requests++;
    } else {
      g_connection_manager_instance_data.close_other_requests++;
    }
