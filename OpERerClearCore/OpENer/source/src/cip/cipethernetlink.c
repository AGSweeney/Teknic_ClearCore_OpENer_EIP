...
EipStatus CipEthernetLinkInit(void) {
  ...
  for (size_t idx = 0; idx < OPENER_ETHLINK_INSTANCE_CNT; ++idx) {
    ...
    g_ethernet_link[idx].interface_state = 0x01;
#if defined(OPENER_ETHLINK_LABEL_ENABLE) && 0 != OPENER_ETHLINK_LABEL_ENABLE
...
{
#if defined(OPENER_ETHLINK_CNTRS_ENABLE) && 0 != OPENER_ETHLINK_CNTRS_ENABLE
  const CipEthernetLinkInterfaceCounters *const cntrs = data;
  for (size_t i = 0; i < 11; i++) {
    EncodeCipUdint(&cntrs->cntr32[i], outgoing_message);
  }
#else
...
{
#if defined(OPENER_ETHLINK_CNTRS_ENABLE) && 0 != OPENER_ETHLINK_CNTRS_ENABLE
  const CipEthernetLinkMediaCounters *const cntrs = data;
...
EipStatus GetAndClearEthernetLink(
  ...
  message_router_response->message.used_message_length = 0;
