...
EipStatus GetAttributeAll(CipInstance *instance,
  ...
  OPENER_TRACE_INFO("GetAttributeAll: class=0x%02X, instance=%d, num_attrs=%d\n",
                    instance->cip_class->class_code,
                    instance->instance_number,
                    instance->cip_class->number_of_attributes);

  if(0 == instance->cip_class->number_of_attributes) {
    OPENER_TRACE_INFO("GetAttributeAll: No attributes for class 0x%02X instance %d\n", 
                      instance->cip_class->class_code, instance->instance_number);
    GenerateGetAttributeSingleHeader(message_router_request,
                                     message_router_response);
    message_router_response->general_status = kCipErrorSuccess;
    message_router_response->size_of_additional_status = 0;
    ...
    for(size_t j = 0; j < instance->cip_class->number_of_attributes; j++) {
      ...
      if( (instance->cip_class->get_all_bit_mask[CalculateIndex(attribute_number)
        ...
        OPENER_TRACE_INFO("GetAttributeAll: Encoding attribute %d (class=0x%02X, instance=%d), data_ptr=%p\n",
                          attribute_number,
                          instance->cip_class->class_code,
                          instance->instance_number,
                          attribute->data);
    ...
    OPENER_TRACE_INFO("GetAttributeAll: Returning %d attributes for class 0x%02X instance %d\n",
                      attributes_returned,
                      instance->cip_class->class_code,
                      instance->instance_number);
  }
...
void EncodeEPath(const void *const data,
  ...
  size_t start_length __attribute__((unused)) = message->used_message_length;
