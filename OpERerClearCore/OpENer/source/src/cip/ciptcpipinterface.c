...
static bool IsIOConnectionActive(void) {
  ...
  while (NULL != node) {
    ...
    if (ConnectionObjectIsTypeIOConnection(connection)) {
      ConnectionObjectState state = ConnectionObjectGetState(connection);
      if (state == kConnectionObjectStateEstablished) {
        OPENER_TRACE_WARN("IsIOConnectionActive: Found established I/O connection (connection_number %u, state %d)\n",
                          connection->connection_number, state);
        return true;
      }
    }
    node = node->next;
...
int DecodeCipTcpIpInterfaceConfiguration( /* Attribute 5 */
	...
	if (IsIOConnectionActive()) {
		OPENER_TRACE_WARN("DecodeCipTcpIpInterfaceConfiguration: Cannot change IP address - active I/O connections detected\n");
		OPENER_TRACE_WARN("DecodeCipTcpIpInterfaceConfiguration: Please close all I/O connections before changing network configuration\n");
	...
	if (kTcpipCfgCtrlStaticIp
		...
		OPENER_TRACE_INFO("DecodeCipTcpIpInterfaceConfiguration: Auto-switching to Static IP mode\n");
		g_tcpip.config_control = (g_tcpip.config_control & ~kTcpipCfgCtrlMethodMask) | kTcpipCfgCtrlStaticIp;
	}
...
int DecodeCipTcpIpInterfaceHostName( /* Attribute 6 */
	...
	int number_of_decoded_bytes = -1;
	          ...
	          *data = tmp_host_name; //write data to attribute
	          ...
	          g_tcpip.status |= kTcpipStatusIfaceCfgPend;
	          message_router_response->general_status = kCipErrorSuccess;
	          number_of_decoded_bytes = 2 + host_name_length;
...
EipStatus CipTcpIpInterfaceInit() {
  ...
  InsertAttribute(instance,
                  ...
                  (CipAttributeDecodeFromMessage)DecodeCipTcpIpInterfaceConfiguration,
  ...
  InsertAttribute(instance,
                  ...
                  kCipString,
                  EncodeCipString,
                  (CipAttributeDecodeFromMessage)DecodeCipTcpIpInterfaceHostName,
                  &g_tcpip.hostname,
                  kGetableSingleAndAll | kNvDataFunc | IFACE_CFG_SET_MODE);
  ...
  InsertGetSetCallback(tcp_ip_class, NvTcpipSetCallback, kPostSetFunc);
