...
EipUint16 SetupIoConnectionOriginatorToTargetConnectionPoint(
  ...
  if( NULL !=
    ...
    int requested_size = ConnectionObjectGetOToTConnectionSize(io_connection_object);
    int data_size = requested_size;
    int diff_size = 0;
    ...
    int assembly_size = ( (CipByteArray *) attribute->data )->length;
    ...
    if( assembly_size != data_size ) {
      /*wrong connection size */
      connection_object->correct_originator_to_target_size =
        assembly_size + diff_size;
      OPENER_TRACE_ERR("SetupIoConnectionOriginatorToTarget: Size mismatch! PLC requested %d bytes, assembly is %d bytes (PLC should request %d bytes)\n",
                       requested_size, assembly_size, assembly_size + diff_size);
...
EipUint16 SetupIoConnectionTargetToOriginatorConnectionPoint(
  ...
  if( NULL !=
    ...
    int requested_size = ConnectionObjectGetTToOConnectionSize(io_connection_object);
    int data_size = requested_size;
    int diff_size = 0;
    /* an assembly object should always have a data attribute. */
    ...
    if( assembly_size != data_size ) {
      ...
      connection_object->correct_target_to_originator_size =
        assembly_size + diff_size;
      OPENER_TRACE_ERR("SetupIoConnectionTargetToOriginator: Size mismatch! PLC requested %d bytes, assembly is %d bytes (PLC should request %d bytes)\n",
                       requested_size, assembly_size, assembly_size + diff_size);
...
#ifdef CLEARCORE
    (int)(intptr_t)g_network_status.udp_io_messaging;
#else
                                  ...
                                  CipCommonPacketFormatData *common_packet_format_data)
{
  ...
  connection_object->socket[direction] =
#ifdef CLEARCORE
        (int)(intptr_t)g_network_status.udp_io_messaging;
#else
        g_network_status.udp_io_messaging;
