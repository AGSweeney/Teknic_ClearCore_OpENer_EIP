...
#ifdef CLEARCORE
  struct udp_pcb *pcb; /**< associated UDP PCB */
#else
...
#ifdef CLEARCORE
    g_delayed_encapsulation_messages[i].pcb = NULL;
#else
...
EipStatus HandleReceivedExplictUdpData(const int socket, const struct sockaddr_in *from_address, const EipUint8 *buffer, const size_t buffer_length,
  ...
  OPENER_TRACE_INFO("encap: UDP packet - command=0x%04X, options=0x%04X, length=%d\n",
                    encapsulation_data.command_code,
                    encapsulation_data.options,
  ...
  if(kEncapsulationHeaderOptionsFlag == encapsulation_data.options) /*TODO generate appropriate error response*/
  {
    if(*number_of_remaining_bytes >= 0) /* check if the message is corrupt: header size + claimed payload size > than what we actually received*/
    {
      ...
      OPENER_TRACE_ERR("encap: UDP packet corrupt - remaining_bytes=%d\n", *number_of_remaining_bytes);
    }
  } else {
    OPENER_TRACE_ERR("encap: UDP packet options flag mismatch - expected=0x%04X, got=0x%04X\n",
                     kEncapsulationHeaderOptionsFlag, encapsulation_data.options);
  }
  return return_value;
}
                                          ...
                                          const EncapsulationData *const receive_data)
{
  ...
  for(size_t i = 0; i < ENCAP_NUMBER_OF_SUPPORTED_DELAYED_ENCAP_MESSAGES; i++) {
#ifdef CLEARCORE
    if(NULL == g_delayed_encapsulation_messages[i].pcb) {
#else
...
#ifdef CLEARCORE
    delayed_message_buffer->pcb = (struct udp_pcb *)(intptr_t)socket;
#else
  ...
  if((0 < protocol_version) && (protocol_version <= kSupportedProtocolVersion) && (0 == option_flag)) { /*Option field should be zero*/
    ...
    for(unsigned int i = 0; i < OPENER_NUMBER_OF_SUPPORTED_SESSIONS; ++i) {
      if(g_registered_sessions[i] == socket) {
        ...
        OPENER_TRACE_ERR(
            "Register Session: Error - A session is already registered at socket %d\n",
            socket);
    ...
    if(kSessionStatusInvalid != session_index) {
      ...
      {
        OPENER_TRACE_ERR("Register Session: No more sessions available\n");
        ...
        OPENER_TRACE_INFO("Register Session: Success - session_handle=%d, socket=%d\n",
                          session_handle, socket);
      }
    }
  } else { /* protocol not supported */
    OPENER_TRACE_ERR("Register Session: Unsupported protocol - version=%d (max=%d), option_flag=0x%04X\n",
                      protocol_version, kSupportedProtocolVersion, option_flag);
...
EipStatus HandleReceivedSendRequestResponseDataCommand(const EncapsulationData *const receive_data, const struct sockaddr *const originator_address,
  ...
  if(receive_data->data_length >= 6) {
    ...
    SessionStatus session_status = CheckRegisteredSessions(receive_data);

    if(kSessionStatusValid == session_status) /* see if the EIP session is registered*/
    {
      return_value = NotifyCommonPacketFormat(receive_data, originator_address, outgoing_message);
    } else { /* received a package with non registered session handle */
      OPENER_TRACE_ERR("SendRequestReplyData: Invalid session handle 0x%08X\n", receive_data->session_handle);
      InitializeENIPMessage(outgoing_message);
      GenerateEncapsulationHeader(receive_data, 0, receive_data->session_handle, kEncapsulationProtocolInvalidSessionHandle, outgoing_message);
      return_value = kEipStatusOkSend; /* TODO: Needs to be here if line with first TODO of this function is adjusted. */
    }
  } else {
    OPENER_TRACE_ERR("SendRequestReplyData: data_length too small: %d < 6\n", receive_data->data_length);
  }
  return return_value;
}
  ...
  for(size_t i = 0; i < ENCAP_NUMBER_OF_SUPPORTED_DELAYED_ENCAP_MESSAGES; i++) {
    ...
    if(kEipInvalidSocket != g_delayed_encapsulation_messages[i].socket) {
      ...
      if(0 >= g_delayed_encapsulation_messages[i].time_out) {
        OPENER_TRACE_INFO("encap: Sending delayed UDP List Identity response\n");
#ifdef CLEARCORE
        ip_addr_t addr;
        ip4_addr_set_u32(&addr, g_delayed_encapsulation_messages[i].receiver.sin_addr.s_addr);
        struct pbuf *tx_buf = pbuf_alloc(PBUF_TRANSPORT, g_delayed_encapsulation_messages[i].outgoing_message.used_message_length, PBUF_RAM);
        if (tx_buf != NULL) {
          memcpy(tx_buf->payload, g_delayed_encapsulation_messages[i].outgoing_message.message_buffer, g_delayed_encapsulation_messages[i].outgoing_message.used_message_length);
          err_t err = udp_sendto(g_delayed_encapsulation_messages[i].pcb, tx_buf, &addr, ntohs(g_delayed_encapsulation_messages[i].receiver.sin_port));
          pbuf_free(tx_buf);
          if (err == ERR_OK) {
            OPENER_TRACE_INFO("encap: Delayed UDP response sent, %d bytes\n", g_delayed_encapsulation_messages[i].outgoing_message.used_message_length);
          } else {
            OPENER_TRACE_ERR("encap: Error sending delayed UDP response: err=%d\n", err);
          }
        } else {
          OPENER_TRACE_ERR("encap: Failed to allocate pbuf for delayed UDP response\n");
        }
        g_delayed_encapsulation_messages[i].pcb = NULL;
#else
        int sent = sendto(g_delayed_encapsulation_messages[i].socket, (char*) g_delayed_encapsulation_messages[i].outgoing_message.message_buffer,
        ...
        if(sent < 0) {
          OPENER_TRACE_ERR("encap: Error sending delayed UDP response\n");
        } else {
          OPENER_TRACE_INFO("encap: Delayed UDP response sent, %d bytes\n", sent);
        }
        g_delayed_encapsulation_messages[i].socket = kEipInvalidSocket;
