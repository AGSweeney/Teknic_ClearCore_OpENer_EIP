...
EipStatus NvdataLoad(void) {
  ...
  EipStatus tcpip_status = NvTcpipLoad(&g_tcpip);
  if (tcpip_status == kEipStatusError) {
    OPENER_TRACE_INFO("NvdataLoad: TCP/IP config not found in EEPROM, using defaults\n");
  }
#endif
  ...
  if ( 0 != (kNvDataFunc & attribute->attribute_flags) ) {
    ...
    if ( 0 == (0x80 & service) ) {
      ...
      if (status == kEipStatusOk) {
        if (attribute->attribute_number == 3 || 
            attribute->attribute_number == 5 || 
            attribute->attribute_number == 6) {
          OPENER_TRACE_INFO("NvTcpipSetCallback: Network configuration saved to NVM. Reboot required to activate.\n");
        }
      }
#endif
    }
